...
def add_persons(backup_dir, add_persons_dir, faces_save_dir, features_path, reset_cache=False):
    """
    ...
    Args:
        ...
        reset_cache (bool): Whether to reset the feature cache and reprocess backup data.
    """
    # Xóa cache nếu được yêu cầu
    if reset_cache and os.path.exists(features_path + '.npz'):
        os.remove(features_path + '.npz')
        print("Đã xóa cache features!")

    # Kiểm tra và xử lý dữ liệu từ thư mục backup
    if reset_cache and os.path.exists(backup_dir) and os.listdir(backup_dir):
        print("Đang xử lý lại dữ liệu từ backup...")
        # Di chuyển dữ liệu từ backup về thư mục add_persons
        for item in os.listdir(backup_dir):
            src_path = os.path.join(backup_dir, item)
            dst_path = os.path.join(add_persons_dir, item)
    ...
    for sub_dir in os.listdir(add_persons_dir):
        ...
        # Nếu thư mục đích đã tồn tại trong backup, xóa nó trước
        if os.path.exists(backup_path):
            shutil.rmtree(backup_path)

        # Di chuyển dữ liệu mới vào backup
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--reset-cache",
        action="store_true",
        help="Reset feature cache and reprocess backup data",
    )
